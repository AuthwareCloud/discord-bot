...
public class Utility : InteractionModuleBase<SocketInteractionContext>
{
    ...
    public async Task CleanAsync()
    {
        ...
        await Context.Interaction.SuccessAsync($"**{safeMessages.Count()}** response(s) have been deleted.", true);
    }
    ...
    public async Task AddDevAsync()
    {
        ...
        {
            await Context.Interaction.ErrorAsync($"You're already a developer.", true);
            return;
        }

        await guildUser.AddRoleAsync(910893494128746518);

        await Context.Interaction.SuccessAsync($"You've been given the **Developer** role.", true);
    }
    ...
    public async Task Base64Decode(IMessage message)
    {
        ...
        {
            await Context.Interaction.ErrorAsync("The message is not a Base64 string.", true);
        }
    }
    ...
    public async Task ClearAsync([Summary("amount", "The amount of messages to clear from the channel")] uint amount)
    {
        ...
        if (!safeMessages.Any())
        {
            await Context.Interaction.ErrorAsync("There is nothing for me to delete.", true);
            return;
        }

        await ((SocketTextChannel) Context.Channel).DeleteMessagesAsync(safeMessages);

        await Context.Interaction.ErrorAsync($"**{safeMessages.Count()}** message(s) have been deleted.", true);
    }
}